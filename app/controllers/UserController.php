...
class UserController extends Controller {
	...
	private function check_password($pw, $confirm)
	{
		...
		else if($pw !== $confirm)
		{
			...
			return $this->f3->get('i18n_password_not_alphanumeric');
		}
		else 
		{
			return "";
	...
	public function create()
	{
		if($this->f3->exists('POST.create'))
		{
			...
			$usercheck = $this->check_availability( $this->f3->get('POST.email'), $this->f3->get('POST.username'));
			if (strlen($pwcheck) > 0)
			{ 
				$this->f3->set('message', $pwcheck);
				$this->f3->set('view','user/create.htm');
			} else if(strlen($usercheck) > 0)
			{	$this->f3->set('message', $usercheck);
				$this->f3->set('view','user/create.htm');
			}
			else{
	...
	public function login()
	{
		...
		else if($this->f3->exists('POST.login')) // OR $this->f3->VERB=='POST'
		{
			...
			else 
			{
				...
				if($this->f3->get('POST.remember_me')){
					$user->rememberme($user->id, 1);
				}
	...
	public function check_availability($email ='',$username='')
	{
		...
		$count1 = 0;
		$user = new User($this->db);
		$count = $user->count(array('email=?', $email));
		$count1 = $user->count(array('username=?', $username));
		if($count1 ){
			return $this->f3->get('i18n_username_taken');
		}else if($count ){
			return $this->f3->get('i18n_email_taken');
		}else 
		{
			return "";
		}
